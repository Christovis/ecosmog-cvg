recursive subroutine amr_step(ilevel,icount)
  ...
  !------------------------------------18-09-14------------------------------------!
  !--- Deleted the following segment and move it to above current if statement ----!
  !------------------------------------18-09-14------------------------------------!
!    if(extradof) then
!       if(ilevel.eq.levelmin .and. aexp.le.2.0d0) then
!          do icell=1,ncoarse+twotondim*ngridmax
...
!    end if
        ...
        do i=ilevel,nlevelmax
           if(i>levelmin) then
              ...
              if(extradof)then
                 ...
                 !----------------------------------------------
                 ! Update ps values on the virtual boundaries:
                 ...
                 if(extradof4) then
                    ...
                    !call make_virtual_fine_dp(bf_src1(1),i)
                    !call make_virtual_fine_dp(bf_src2(1),i)
                    !call make_virtual_fine_dp(bf_src3(1),i)
                    !call make_virtual_fine_dp(bf_src4(1),i)
                    do idim=1,ndim
  ...
  if(poisson) then
     ...
     ! Compute the extradof and its gradient
     if(extradof) then
