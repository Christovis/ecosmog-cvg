...
subroutine load_balance
  ...
  use extradof_commons,ONLY: sf,sf_grad,sf_lp,cbf,sf_src !1,bf_src2,bf_src3,bf_src4
  use bisection
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h' 
...
subroutine defrag
  ...
  if(ray.and.ray_initialized) then                                                      ! RAY_RAMSES
     do iray=1,ray_nrays                                                                ! RAY_RAMSES
        ray_grid(iray,1) = cpu_map2(ray_grid(iray,1))                                   ! RAY_RAMSES
     enddo                                                                              ! RAY_RAMSES
  endif                                                                                 ! RAY_RAMSES
  ...
  if(extradof) then
     ...
     do ind=1,twotondim
        ...
        do ilevel=1,nlevelmax
           do ibound=1,nboundary+ncpu
              ...
              if(ncache>0)then
                 igrid=istart
                 do i=1,ncache
                    hilbert_key(ngrid2+i)=real(sf(iskip2+igrid),kind=qdp)
                    igrid=next(igrid)
                 end do
                 ngrid2=ngrid2+ncache
              end if
        ...
        do igrid=1,igridmax
           sf(iskip2+igrid)=real(hilbert_key(igrid),kind=8)
        end do
     end do
     ...
     do ind=1,twotondim
        ...
        do ilevel=1,nlevelmax
           do ibound=1,nboundary+ncpu
              ...
              if(ncache>0)then
                 igrid=istart
                 do i=1,ncache
                    hilbert_key(ngrid2+i)=real(sf_src(iskip2+igrid),kind=qdp)
                    igrid=next(igrid)
                 end do
                 ngrid2=ngrid2+ncache
              end if
        ...
        do igrid=1,igridmax
           sf_src(iskip2+igrid)=real(hilbert_key(igrid),kind=8)
        end do
     end do
     ...
     if(extradof4) then
        do ind=1,twotondim
           ...
           do ilevel=1,nlevelmax
              do ibound=1,nboundary+ncpu
                 ...
                 if(ncache>0)then
                    igrid=istart
                    do i=1,ncache
                       hilbert_key(ngrid2+i)=real(sf_lp(iskip2+igrid),kind=qdp)
                       igrid=next(igrid)
                    end do
                    ngrid2=ngrid2+ncache
                 end if
           ...
           do igrid=1,igridmax
              sf_lp(iskip2+igrid)=real(hilbert_key(igrid),kind=8)
           end do
        end do
        ...
        !            igrid=istart
        !            do i=1,ncache
        !               hilbert_key(ngrid2+i)=real(bf_src1(iskip2+igrid),kind=qdp)
        !               igrid=next(igrid)
        !            end do
        !            ngrid2=ngrid2+ncache
        !         end if
        ...
        !   end do
        !   do igrid=1,igridmax
        !      bf_src1(iskip2+igrid)=real(hilbert_key(igrid),kind=8)
        !   end do
        !end do
        ...
        !            igrid=istart
        !            do i=1,ncache
        !               hilbert_key(ngrid2+i)=real(bf_src2(iskip2+igrid),kind=qdp)
        !               igrid=next(igrid)
        !            end do
        !            ngrid2=ngrid2+ncache
        !         end if
        ...
        !   end do
        !   do igrid=1,igridmax
        !      bf_src2(iskip2+igrid)=real(hilbert_key(igrid),kind=8)
        !   end do
        !end do
        ...
        !            igrid=istart
        !            do i=1,ncache
        !               hilbert_key(ngrid2+i)=real(bf_src3(iskip2+igrid),kind=qdp)
        !               igrid=next(igrid)
        !            end do
        !            ngrid2=ngrid2+ncache
        !         end if
        ...
        !   end do
        !   do igrid=1,igridmax
        !      bf_src3(iskip2+igrid)=real(hilbert_key(igrid),kind=8)
        !   end do
        !end do
        ...
        !            igrid=istart
        !            do i=1,ncache
        !               hilbert_key(ngrid2+i)=real(bf_src4(iskip2+igrid),kind=qdp)
        !               igrid=next(igrid)
        !            end do
        !            ngrid2=ngrid2+ncache
        !         end if
        ...
        !   end do
        !   do igrid=1,igridmax
        !      bf_src4(iskip2+igrid)=real(hilbert_key(igrid),kind=8)
        !   end do
        !end do
     ...
     do idim=1,ndim
        do ind=1,twotondim
           ...
           do ilevel=1,nlevelmax
              do ibound=1,nboundary+ncpu
                 ...
                 if(ncache>0)then
                    igrid=istart
                    do i=1,ncache
                       hilbert_key(ngrid2+i)=real(sf_grad(iskip2+igrid,idim),kind=qdp)
                       igrid=next(igrid)
                    end do
                    ngrid2=ngrid2+ncache
                 end if
           ...
           do igrid=1,igridmax
              sf_grad(iskip2+igrid,idim)=real(hilbert_key(igrid),kind=8)
           end do
        end do
     end do
     ...
     if(extradof4) then
        do idim=1,ndim
           do ind=1,twotondim
              ...
              do ilevel=1,nlevelmax
                 do ibound=1,nboundary+ncpu
                    ...
                    if(ncache>0)then
                       igrid=istart
                       do i=1,ncache
                          hilbert_key(ngrid2+i)=real(cbf(iskip2+igrid,idim),kind=qdp)
                          igrid=next(igrid)
                       end do
                       ngrid2=ngrid2+ncache
                    end if
              ...
              do igrid=1,igridmax
                 cbf(iskip2+igrid,idim)=real(hilbert_key(igrid),kind=8)
              end do
           end do
        end do
     end if
