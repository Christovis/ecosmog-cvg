...
subroutine refine_fine(ilevel)
  ...
  if(ray.and.ray_initialized) ex_father = father                                        ! RAY_RAMSES
...
! use extradof_commons,ONLY:sf_grad,sf,sf_src,sf_old
!#else
! use poisson_commons, ONLY:f,phi,phi_old
! use extradof_commons,ONLY:sf_grad,sf,sf_src
!#endif
#ifdef RT
  ...
  real(dp),dimension(1:nvector,1:twotondim),save::u3
...
!#ifndef OLDINTERP
              if(allocated(phi_old)) phi_old(iskip+ind_grid_son(i))=phi_old(ind_fathers(i,0))
!#endif
           end do
        end do
     end if
     ...
     if(extradof) then
        ...
        ! Scatter values in parent cells to children cells
        ! These quantities are actually unused in Galileon simulations
        do j=1,twotondim
           iskip=ncoarse+(j-1)*ngridmax
           do idim=1,ndim
              do i=1,nn
                 sf_grad(iskip+ind_grid_son(i),idim)=sf_grad(ind_fathers(i,0),idim)
              end do
           end do
           do i=1,nn
              sf(iskip+ind_grid_son(i))=sf(ind_fathers(i,0))
              sf_src(iskip+ind_grid_son(i))=sf_src(ind_fathers(i,0))
!#ifndef OLDINTERP
              if(allocated(sf_old)) sf_old(iskip+ind_grid_son(i))=sf_old(ind_fathers(i,0))
!#endif
           end do
        end do
     end if
...
!#ifndef OLDINTERP
           if(allocated(phi_old)) phi_old(ind_cell_son(i))=0.0D0
!#endif
        end do
        do idim=1,ndim
           do i=1,nn
...
!#ifndef OLDINTERP
           if(allocated(sf_old)) sf_old(ind_cell_son(i))=0.0D0
!#endif
        end do
        do idim=1,ndim
           do i=1,nn
              sf_grad(ind_cell_son(i),idim)=0.0D0
           end do
        end do
     end if
