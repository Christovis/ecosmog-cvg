...
recursive subroutine amr_step(ilevel,icount)
  ...
  !real(dp) :: xs,ys,zs
  !integer  :: ix,iy,iz
  !integer  :: ngrid,igrid_amr,icell_amr
  ...
  !real(dp) :: delta_in,delta_out,rr,R0,pii         ! 3D spherical test

  if(numbtot(1,ilevel)==0) return
  if(verbose) write(*,999) icount,ilevel

  ! Parameters for 1D sine test
  !Amp = 0.00000005d0
  !pii = 4.0d0*datan(1.0d0)
  ! Parameters for 1D gauss test
  !Amp = 0.000005d0
  !sigma = 0.09
  !alpha = 0.01
  ! Parameters for 3D spherical test
  !delta_in = 190.9d0  !23.77d0
  !delta_out = -0.1d0
  !R0 = 0.05d0
  !pii = 4.0d0*datan(1.0d0)
  ...
  if(poisson) then
     ...
     !      
     !      !! 3D spherical test of longitudinal/chi-mode cv-Galileon
     !      !rr = dsqrt( &
     !      !   (xs-(0.5D0))**2 + &
     !      !   (ys-(0.5D0))**2 + &
     !      !   (zs-(0.5D0))**2 &
     ...
     !      ! 1D test
     !      if(zs>0.5D0 .and. zs<0.5D0+1.0D0/256.0D0 .and. ys.eq.xs) then
     !         write(*,'(A,11(F16.10,2x))') 'cvg ', aexp, beta_cvg, rc_cvg, xs, ys, zs, rho(icell_amr), sf(icell_amr), cbf(icell_amr,1), cbf(icell_amr,2), cbf(icell_amr,3)
     !      end if
     !      
     !      ! Circular plots
     ...
     !      !   ys>0.5D0 .and. &
     !      !   zs>0.5D0 .and. &
     !      !   ys<0.5D0+1.0D0/128.0D0 .and. &
     !      !   zs<0.5D0+1.0D0/.0D0) then
